...
def get_gemini_voices() -> list[str]:
    ...
    """
    # 添加gemini:前缀；Gemini TTS当前仅接受固定voiceName枚举
    return [f"gemini:{voice}-Neutral" for voice in GEMINI_TTS_SUPPORTED_VOICES]
...
def gemini_tts(
    ...
    """Use Gemini TTS and return subtitle timing metadata."""
    try:
        import base64
        import io
        ...
        if voice_name not in GEMINI_TTS_SUPPORTED_VOICES:
            logger.warning(
                f"Unsupported Gemini voice '{voice_name}', fallback to 'zephyr'"
            )
            voice_name = "zephyr"
        ...
        for i in range(3):
            ...
            if response.status_code != 200:
                logger.error(
                    f"Gemini TTS HTTP {response.status_code}: {response.text[:500]}"
                )
                continue
            ...
            if not candidates:
                logger.error("No candidates received from Gemini TTS")
                continue
